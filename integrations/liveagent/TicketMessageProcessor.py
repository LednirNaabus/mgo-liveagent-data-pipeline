...
class TicketMessageProcessor:
    """
    Docstring for TicketMessageProcessor
    """
    def __init__(self, client: LiveAgentClient, bq_client: BigQueryUtils):
        self.client = client
    ...
    async def _load_agents_from_bq(self):
        if not self.bq_client:
            raise ValueError("No BigQuery client set.")

        try:
            query = """SELECT id, name FROM `{}.{}.agents`""".format(PROJECT_ID, DATASET_ID)
    ...
    async def preload_users_from_bq(self, user_ids: Set[str]):
        if not self.bq_client or not user_ids:
            return
        ...
        try:
            ...
            AND id IS NOT NULL
            """.format(PROJECT_ID, DATASET_ID, user_ids_str)
            df = self.bq_client.query_to_dataframe(query)
    ...
    def _determine_sender_receiver(self, message_data: Dict) -> Dict[str, str]:
        ...
        if message_userid in self.agent_cache:
            agent_name = self.agent_cache[message_userid].get("name", "Uknown Agent")
            return {
    ...
    async def process_messages_with_metadata(
        ...
        for msg in messages_data:
            ...
            agent_name = agent_info["name"] if agent_info else "Uknown Agent"
