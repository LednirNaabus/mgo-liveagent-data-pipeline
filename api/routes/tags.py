...
from integrations.liveagent.models import TAGS_SCHEMA
...
@router.post("/tags")
async def process_tags(request: Request, background_tasks: BackgroundTasks) -> JSONResponse:
    session = get_aiohttp_session(request=request)
    bq_client = get_bq_utils(request=request, table_id="tags_test")
    gateway = get_channel_gateway(session=session, bq_client=bq_client)
    extractor = ConvoData(gateway=gateway)
    response = await extractor.fetch_tags()
